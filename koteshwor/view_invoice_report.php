			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
					...
					<?php
						...
						$sql = mysql_query("SELECT * FROM billing_sales_order WHERE sales_id='$sales_id' LIMIT 1") or die(mysql_error());
								...
								<div class="row">
								 ...
								 <table class="table table-bordered table-striped table-condensed mb-none">
                                	...
                                	?>
	                                	...
	                                	<?php
											$sql = mysql_query("SELECT *, sum(billing_qty) as qty_total, sum(billing_qty*product_price) as per_total FROM billing_sales_order WHERE sales_id='$sales_id' GROUP BY product_name") or die(mysql_error());
							                $counter=0;
							                while($row = mysql_fetch_object($sql))
							                {
							                	print '<tr>';
						            	...
						            	?>
	                               		<?php } ?>
										<?php 
			                                $sql = mysql_query("SELECT sales_id, sum(product_price) AS gtotal FROM billing_sales_order WHERE sales_id='$sales_id'") or die(mysql_error());
												while($row = mysql_fetch_object($sql))
			                                {
					                            print'<tr>';
												print'<th></th>';
									...
									?>
										...
										<?php
											$sql = mysql_query("SELECT *,cast(sum(product_price)-(SELECT discount_rate FROM billing_sales WHERE sales_id='$sales_id') AS decimal(18,2)) AS discountprice FROM billing_sales_order WHERE sales_id='$sales_id'") or die(mysql_error());
								                while($row = mysql_fetch_object($sql))
								                {
