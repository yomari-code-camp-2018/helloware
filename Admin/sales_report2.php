						...
						<section class="panel">
							...
							<div class="tabs">
								<ul class="nav nav-tabs tabs-primary">
									<li class="active">
										<a href="#cash" data-toggle="tab">Sales Transaction</a>
									</li>
								</ul>
								<div class="tab-content">
									<div id="cash" class="tab-pane active">
									
										<form class="form-horizontal" action="sales_transaction_costumer.php" method="post">
											<?php
					                        if(isset($_POST["search"])){
					                          $client_name=$_POST["client_name"];
					                          $sql="SELECT  * FROM billing_costumer WHERE client_name LIKE '%$client_name%'";
					                        }
					                        else{
					                        $sql="SELECT * FROM billing_costumer";
					                        }
											...
											<select class="form-control" name="client_name" required>
													<option value="" selected disabled>Select Costumer Name</option>
													<?php 
									                    	$option = mysql_query("SELECT * FROM billing_costumer");
										...
										<table class="table table-bordered table-striped mb-none" id="datatable-editable">
											...
											<tbody>
												<?php
									                ...
									                $sql = mysql_query("SELECT *, sum(item_price * item_quantity) AS sum1, sum(sales_paid*item_quantity) as sum2, date(sales_tran_date) AS date1 FROM billing_sales_transaction GROUP BY sales_id") or die(mysql_error());
									                while($row = mysql_fetch_object($sql))
									                {
									                	print '<tr class="gradeA">';
									                	print '<td>'.$row->date1.'</td>';
									                	print '<td><a href="sales_summary_print.php?sales_id='.$row->sales_id.'">'.$row->sales_id.'|'.$row->transaction_type.'</a></td>';
