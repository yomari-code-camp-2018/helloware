											...
											<table class="table table-bordered table-striped mb-none" id="datatable-editable">
												...
												<tbody>
													<?php
										                ...
										                $sql = mysql_query("SELECT *, date(billing_date) as date1 FROM `billing_sales` WHERE DATE(`billing_date`) = CURDATE();") or die(mysql_error());
										                while($row = mysql_fetch_object($sql))
										                {
										                	print '<tr class="gradeA">';
										                	print '<td><a href="running_sales_edit.php?sales_id='.$row->sales_id.'">SALES_'.$row->sales_id.'</a></td>';
										                	print '<td>'.$row->member_card.'</td>';
										                	print '<td>'.$row->member_name.'</td>';
										                	print '<td>'.$row->billing_date.'</td>';
										                	print '</tr>';
										                } 
										            ?>
												</tbody>
